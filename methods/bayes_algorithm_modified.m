...
for y=1:gridmax_y
    for x=1:gridmax_x
        ...
        if(ENABLE_CORRECTION==1)
            ...
            if(count~=1)
                prob_last = prob_out{end};
                correction_prob_sum=0;
                for esty_prev=1:gridmax_y
                    for estx_prev=1:gridmax_x
                        ...
                        correction_prob_sum=correction_prob_sum+(correction_prob*prob_last(estx_prev,esty_prev));
                    end
                end

                prob_dist(x,y)=prob_dist(x,y)*correction_prob_sum;
            end
        end
